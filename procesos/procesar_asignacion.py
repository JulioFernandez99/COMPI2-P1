...
def procesar_asignacion(instr, ts):
    ...
    if exp!="ERARA91":#este error es para cuando se intenta acceder a una posicion que no existe en el array
        ...
        # elif vls.tipo ==TIPO_DATO.ARRAY and type(exp) == list and vls.constante == False:
        #     ts.actualizar(id, exp)
        # elif isinstance(instr.exp, LlamadaFuncionNativa):
        #     ts.actualizar(id, exp)
        # elif exp == "RARR":
        #     print("Error: No es posible asignar valor a esta variable, no existe la posicion en el array")  
